    ...
    try {
        if (isset($_POST['add_technician'])) {
            ...
            if (empty($name) || empty($phone) || empty($specialization)) {
                $error = 'Name, phone, and specialization are required fields.';
            } else {
                $db = Database::getInstance();
                $sql = "INSERT INTO technicians (full_name, phone, specialization, experience_years, status) 
                        VALUES (?, ?, ?, ?, 'available')";
                $stmt = $db->executeQuery($sql, [$name, $phone, $specialization, $experience_years]);
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <div class="header">
                <h1><i class="fas fa-user-cog me-2"></i>Technician Management</h1>
                <p>Manage and monitor all technicians in the EV Mobile Station network</p>
            </div>
                ...
                <?php foreach ($filtered_technicians as $technician): ?>
                    <div class="technician-card">
                        ...
                        <div class="technician-actions">
                            ...
                            <button class="action-btn delete-btn" onclick="deleteTechnician(<?php echo $technician['id']; ?>)">
